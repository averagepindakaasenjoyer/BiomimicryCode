...
def live_camera_data(camera_index_left, camera_index_right, picamera=picamera):
    """
    Capture live video from two cameras and prepare frames for depth estimation.
    returns: left_frame_gray, right_frame_gray
    """
    if picamera:
        # Use PiCamera to capture two consecutive frames (note: this is not stereo)
        camera = PiCamera()
        ...
        try:
            ...
            left_frame = raw_capture.array.copy()
            raw_capture.truncate(0)

            # Capture second frame
            camera.capture(raw_capture, format="bgr")
            right_frame = raw_capture.array.copy()
            raw_capture.truncate(0)
        finally:
            camera.close()
    else:
