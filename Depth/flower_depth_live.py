...
def main():
    ...
    for cap in [cap_left, cap_right]:
        cap.set(cv2.CAP_PROP_EXPOSURE, -4)  # Between default -6 (too bright) and -8 (too dark)
        cap.set(cv2.CAP_PROP_BRIGHTNESS, -5)  # Slight brightness reduction
    ...
    try:
        while True:
            ...
            if not paused:
                ...
                # Prepare display frame (left feed with detections)
                ...
                display_frame = frame_l.copy()
                cv2.putText(display_frame, "PAUSED", (display_frame.shape[1]//2 - 80, display_frame.shape[0]//2),
                           cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)

            # Display left (overlay) and right feeds in separate windows at capture resolution
            cv2.imshow("Left Feed", display_frame)
            cv2.imshow("Right Feed", frame_r)
