...
CUSTOM_FOLDER = "stereo_capture_20260122_160640"
...
baseline_expected = 0.16  # meters

if baseline_expected == 0.08:
    print("[INFO] Using 8CM calibration parameters")
    if CUSTOM_FOLDER:
        # Use custom captured images
        CALIB_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../Cam_Params/stereo_charuco_calibration_8cm_rico.npz'))
        if SWAP_CAMERAS:
        ...
        # Use calibration images from 8CM set
        CALIB_PATH = os.getenv('STEREO_PARAMS_8CM_RICO')
        LEFT_IMG_PATH = os.path.join(os.getenv('CALIB_IMGS_8CM_DIR'), "camera_0_0.jpg")
        RIGHT_IMG_PATH = os.path.join(os.getenv('CALIB_IMGS_8CM_DIR'), "camera_2_0.jpg")
        print("Using calibration images (8CM)")
elif baseline_expected == 0.16:
    if CUSTOM_FOLDER:
        # Use custom captured images
        ...
        CALIB_PATH = os.getenv('STEREO_PARAMS_16CM')
...
# Rotate images to align cameras (opposite 90 degree rotations)
