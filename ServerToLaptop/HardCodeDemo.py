...
def move_motor_cm(motor_name, dist_cm, hold_arm=True, verbose=True):
    """
    Programmatic version of the 'movecm' command.
    Applies boundary limits and updates position tracking.
    """
    global arm_is_at_bottom

    if motor_name not in ["rails", "main", "arm"]:
        raise ValueError(f"move_motor_cm: invalid motor '{motor_name}'")

    original_dist_cm = dist_cm

    # Enforce boundary limits BEFORE conversion to steps
    if motor_name == "rails":
        ...
        # Arm bottom offset compensation (same logic as movecm branch)
        if arm_is_at_bottom and dist_cm > 0:
            adjusted_dist_cm = dist_cm + ARM_BOTTOM_OFFSET_CM
            if verbose:
                print(f"[DEMO] Arm at bottom, adding {ARM_BOTTOM_OFFSET_CM} cm offset")
                print(f"[DEMO] Adjusted distance: {dist_cm:.2f} cm -> {adjusted_dist_cm:.2f} cm")
    ...
    # Convert cm -> steps (use arm-specific calibration where appropriate)
    if motor_name == "arm":
    ...
    if motor_name == "arm" and steps != 0 and hold_arm:
        motor_command["_hold_motors"] = ["arm"]

    if verbose:
        print(f"[DEMO] Moving {motor_name}: requested ~{original_dist_cm:.2f} cm, "
              f"allowed ~{dist_cm:.2f} cm -> {steps} steps")
...
def scripted_demo_worker():
    """
    Hardcoded demo sequence:
    - Reset to origin
    - Move to a few positions in a pattern
    - Move arm down, pollinate, move arm back up
    - Return to origin
    """
    ...
    print("\n[DEMO_SCRIPT] Starting scripted demo sequence...")
    reset_position()
    print_position()

    try:
        # Example sequence: you can adjust these numbers freely

        # 1) Move to front-left corner
        if demo_stop_flag: return
        print("[DEMO_SCRIPT] Moving to front 10 cm (X = 0, Y = 10)")
        move_motor_cm("main", 10)   # forward 18 cm
        time.sleep(0.5)

        # 2) Move arm down toward flower area
        if demo_stop_flag: return
        print("[DEMO_SCRIPT] Lowering arm 15'//'-cm")
        move_motor_cm("arm", -15.0, hold_arm=True)
        time.sleep(0.5)

        # 3) Pollinate (vibration + Van de Graaf)
        if demo_stop_flag: return
        print("[DEMO_SCRIPT] Pollinating...")
        pollinate(client_socket,
                  vibrate_duration_ms=500,
                  van_de_graaf_duration_ms=500,
                  repeat=2)
        time.sleep(0.5)

        # 4) Raise arm back up
        if demo_stop_flag: return
        print("[DEMO_SCRIPT] Raising arm 15cm")
        move_motor_cm("arm", 15.0, hold_arm=True)
        time.sleep(0.5)

        # 5) Move to another point (front-right)
        if demo_stop_flag: return
        print("[DEMO_SCRIPT] Moving to front-right (Y=18cm, X=0cm)")
        # Rails from 45 -> 0 (move right by -45)
        move_motor_cm("rails", -45.0)
        time.sleep(0.5)

        # Optional: another pollination cycle here
        if demo_stop_flag: return
        print("[DEMO_SCRIPT] Optional second pollination")
        pollinate(client_socket, 400, 400, repeat=1)
        time.sleep(0.5)

        # 6) Return to origin (rear-right: Y=0, X=0)
        if demo_stop_flag: return
        print("[DEMO_SCRIPT] Returning to origin")
        # From current Y (~18) back -18
        move_motor_cm("main", -18.0)
        time.sleep(0.5)

        print("[DEMO_SCRIPT] Scripted demo complete.")

    except Exception as e:
        print(f"[DEMO_SCRIPT] Error: {e}")

    finally:
        # Stop motors (if you want) and mark demo as done
        send_motor_command(client_socket, {})
        demo_mode = False
        print("[DEMO_SCRIPT] Scripted demo stopped")
# =============================
# Command Interface
# =============================
def print_help():
    ...
    print("  demo_script              - Start hardcoded scripted demo sequence")
...
def parse_and_execute_command(cmd_input):
    ...
    try:
        ...
        elif command == "demo_script":
            if demo_mode:
                print("[COMMAND] A demo is already running! (auto or scripted)")
                return

            print("[COMMAND] Starting scripted demo sequence...")
            demo_stop_flag = False
            demo_mode = True
            demo_thread = threading.Thread(target=scripted_demo_worker, daemon=True)
            demo_thread.start()
