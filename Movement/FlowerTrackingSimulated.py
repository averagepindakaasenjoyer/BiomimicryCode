"""
Flower tracking with stereo depth estimation - SIMULATION MODE

Simulates motor control without hardware for testing vision algorithms.
Displays motor commands that would be executed on actual hardware.

Controls:
  - ESC: Exit
  - SPACE: Pause/resume
  - 'q': Quit
  - 'r': Reset simulated robot position
"""
...
class SimulatedRobot:
    def __init__(self):
        self.position_x = 0.0  # cm
        self.position_y = 0.0  # cm
        self.arm_height = 0.0  # cm
        self.total_steps_x = 0
        self.total_steps_y = 0
        self.total_steps_arm = 0
        
    def move(self, motor_name, steps):
        """Simulate motor movement."""
        if motor_name == "rails":
            self.position_x += steps / STEPS_PER_CM
            self.total_steps_x += abs(steps)
        elif motor_name == "main":
            self.position_y += steps / STEPS_PER_CM
            self.total_steps_y += abs(steps)
        elif motor_name == "arm":
            self.arm_height += steps / STEPS_PER_CM_ARM
            self.total_steps_arm += abs(steps)
    ...
    def reset(self):
        ...
        self.position_y = 0.0
        self.arm_height = 0.0
        print("[ROBOT] Position reset to origin")
    ...
    def get_status(self):
        ...
        return {
            ...
            'y': self.position_y,
            'arm': self.arm_height,
            'total_steps': self.total_steps_x + self.total_steps_y + self.total_steps_arm
        }

robot = SimulatedRobot()

# Motor logical directions
direction_dict = {
    "front": [("main", 1), ("main", 1)],
    "rear": [("main", -1), ("main", -1)],
...
# Step delay (for timing simulation)
STEP_DELAY = 0.01
...
# =============================
YOLO_MODEL_PATH = os.path.join(os.path.dirname(__file__), "../current_best_yolo.pt")
...
def motor_step_worker_simulated(motor_name, steps, step_delay=STEP_DELAY):
    """Simulate motor movement with timing."""
    if steps == 0:
        return
    
    direction = "FORWARD" if steps > 0 else "BACKWARD"
    abs_steps = abs(int(steps))
    distance_cm = abs_steps / (STEPS_PER_CM if motor_name != "arm" else STEPS_PER_CM_ARM)
    
    print(f"[MOTOR] {motor_name}: {direction} {abs_steps} steps ({distance_cm:.2f} cm)")
    
    # Update simulated position
    robot.move(motor_name, steps)
    
    # Simulate execution time
    time.sleep(abs_steps * step_delay * 0.1)  # 10x faster than real
...
def main_loop():
    ...
    print("\n" + "="*60)
    print("SIMULATION MODE - Motor movements will be printed")
    print("="*60 + "\n")
    ...
    try:
        while True:
            ...
            if target_det is None:
                print("[INFO] No valid flower detected, searching...")
                
                # Slow backward movement to search
                slow_steps = int(0.5 * STEPS_PER_CM)
                motor_step_worker_simulated("main", slow_steps)
                
                if SHOW_DEBUG:
                    display_frame = cv2.resize(frame_l, (DISPLAY_WIDTH, DISPLAY_HEIGHT))
                    cv2.putText(display_frame, "Searching for flowers...", (50, 50),
                               cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    
                    # Add robot status overlay
                    status = robot.get_status()
                    cv2.putText(display_frame, f"Pos: X={status['x']:.1f} Y={status['y']:.1f} Arm={status['arm']:.1f} cm", 
                               (10, DISPLAY_HEIGHT - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                    
                    cv2.imshow("Flower Tracking (SIMULATED)", display_frame)
            ...
            print(f"\n[INFO] Target flower at dx={dx:.0f}px, dy={dy:.0f}px, depth={depth_m:.3f}m")
            ...
            if plan:
                print("[MOVEMENT] Executing rail movements:")
                for motor_name, steps in plan.items():
                    if steps != 0:
                        motor_step_worker_simulated(motor_name, steps)
            
            # Adjust arm based on depth if well-centered
            if abs(dx) < 30 and abs(dy) < 30:
                target_depth_m = 0.40
                arm_steps = convert_depth_to_arm_steps(depth_m, target_depth_m)
                
                if abs(arm_steps) > 10:
                    print(f"[ARM] Adjusting height to reach target depth:")
                    motor_step_worker_simulated("arm", arm_steps)
            ...
            if SHOW_DEBUG:
                ...
                if target_det:
                    target_x = int(flower_center_x)
                    target_y = int(flower_center_y)
                    cv2.line(display_frame, (w//2, h//2), (target_x, target_y), (0, 255, 255), 2)
                    cv2.circle(display_frame, (target_x, target_y), 10, (0, 255, 0), -1)
                
                # Info overlay
                cv2.putText(display_frame, "SIMULATION MODE", (10, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                cv2.putText(display_frame, f"Detections: {len(detections)}", (10, 60),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(display_frame, f"Offset: ({dx:.0f}, {dy:.0f})px", (10, 90),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                # Robot status overlay
                status = robot.get_status()
                cv2.rectangle(display_frame, (10, h - 80), (500, h - 10), (0, 0, 0), -1)
                cv2.putText(display_frame, f"Robot Position:", (15, h - 60),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                cv2.putText(display_frame, f"X: {status['x']:+.1f} cm  Y: {status['y']:+.1f} cm  Arm: {status['arm']:+.1f} cm", 
                           (15, h - 35), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                cv2.putText(display_frame, f"Total steps: {status['total_steps']}", 
                           (15, h - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        ...
        print("="*60)
        print(f"Final robot position:")
        print(f"  X: {status['x']:+.2f} cm")
        print(f"  Y: {status['y']:+.2f} cm")
        print(f"  Arm height: {status['arm']:+.2f} cm")
        print(f"Total steps executed: {status['total_steps']}")
        print("="*60 + "\n")

if __name__ == "__main__":
    print("=" * 60)
    print("Flower Tracking - SIMULATION MODE")
    print("=" * 60)
    print("Motor movements will be printed to console.")
    print("\nControls:")
    print("  q or ESC: Quit")
