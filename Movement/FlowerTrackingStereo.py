"""
Flower tracking with stereo depth estimation and robotic arm control.

Combines stereo vision flower detection (YOLO) with motor control to:
1. Detect flowers using YOLO on stereo camera pair
2. Estimate depth using stereo vision
3. Move robot to center on detected flower
4. Adjust arm height based on flower depth
...
def select_target_flower(detections, depth_stats_list, frame_width, frame_height):
    ...
    for i, (det, depth_stats) in enumerate(zip(detections, depth_stats_list)):
        ...
        score = depth * 0.7 + dist_from_center * 0.0003  # weight depth more heavily
        ...
        while True:
            ...
            if target_det is None:
                ...
                if motor_obj is not None:
                    motor_step_worker(motor_obj, slow_steps, step_delay=STEP_DELAY * 2)
                ...
                if SHOW_DEBUG:
                    ...
                    cv2.imshow("Flower Tracking", display_frame)
            ...
            print(f"[INFO] Target flower at dx={dx:.0f}px, dy={dy:.0f}px, depth={depth_m:.3f}m")
            ...
            if plan:
                ...
                for motor_name, steps in plan.items():
                    ...
                    if motor_obj is None or steps == 0:
                        continue
            ...
            if SHOW_DEBUG:
                ...
                cv2.putText(display_frame, f"Offset: ({dx:.0f}, {dy:.0f})px", (10, 60),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Flower Tracking with Stereo Depth & Motor Control")
    print("=" * 60)
